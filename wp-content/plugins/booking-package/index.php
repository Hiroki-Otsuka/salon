	...
	class BOOKING_PACKAGE {
		...
		public function __construct($shortcodes = 0) {
			...
			**/
			...
			if (isset($_GET['booking_package_bookingReminder'])) {
				
				add_action('init', array($this, 'do_booking_notification'));
		...
		**/
		...
		public function get_shortcode_count( $content, $tag ) {
		...
		public function localizeScript($mode){
			...
			if ($mode == 'adomin') {
				...
				if ($this->bookingReminder == 0) {
					...
					unset($elementForCalendarAccount['bookingReminder']);
				...
				$localize_script = array(
					...
					'debug' => $this->dubug_javascript,
					'bookingReminder' => $this->bookingReminder,
				);
